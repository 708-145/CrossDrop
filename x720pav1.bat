@echo Simple realtime 720p (1K) AOM AV1 encoder for any input.
...
@if exist %ffmpegpath% (
  %ffmpegpath% -i %1 -filter:v hqdn3d=4.0:3.0:6.0:4.5 -sws_flags lanczos -s hd720 -c:v libaom-av1 -cpu-used 8 -row-mt 1 -tiles 4x2 -crf 24 -b:v 0 -c:a libopus -b:a 128k "%~d1%~p1%~n1_720pav1.mkv"
  pause
) else (
