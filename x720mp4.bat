...
@echo %ffmpegpath% -i %1 -f mp4 -filter:v hqdn3d=4.0:3.0:6.0:4.5 -sws_flags lanczos -c:v libx264 -s hd720 -crf 21 -preset medium -c:a aac -strict -2 -b:a 96k "%~d1%~p1%~n1_720p.mp4"

@if exist %ffmpegpath% (
  rem start /belownormal 
  %ffmpegpath% -i %1 -f mp4 -filter:v hqdn3d=4.0:3.0:6.0:4.5 -sws_flags lanczos -c:v libx264 -s hd720 -crf 21 -preset medium -c:a aac -strict -2 -b:a 96k "%~d1%~p1%~n1_720p.mp4"
) else (
